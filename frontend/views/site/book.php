    ...
    <?php foreach ($books as $book): ?>
            <div class="col-md-3">
                <div class="card" style="width: 18rem; margin-top: 8rem">
                    ...
                    <img src="<?= Html::encode(Url::to('/lms/backend/web/' . $book->bookPhoto)) ?>" class="card-img-top" alt="<?= Html::encode($book->bookName) ?>">


                    <div class="card-body">
                        <h5 class="card-title"><?= Html::encode($book->bookName) ?></h5>
                        <?php
                                        $category = Category::findOne($book->categoryId);
                                        $author = Author::findOne($book->authorId);

                        ?>
                       <p class="card-text"><?= Html::encode($category->categoryName) ?></p>
                       <p class="card-text"><?= Html::encode($author->authorName) ?></p>
